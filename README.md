...
即可保留原来的线路、端口、Path、UUID 和 EasyTier IP，只生成新的加入有效期。
...
推送 `v0.17.1` 形式的 Git 标签后，GitHub Actions 会运行完整测试、交叉编译两个 Linux 架构并创建 Release。构建使用 `CGO_ENABLED=0`，目标机不需要额外运行库。
